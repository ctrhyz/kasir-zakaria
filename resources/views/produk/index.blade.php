...
<body style="background-color: rgba(0, 12, 27, 1)">
    ...
    <main class="mb-5">
                            ...
                            <form action="/produk/simpan" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" style="background-color: rgba(0, 46, 107, 0.60)"
                                        ...
                                        required autofocus id="ProdukID">
                                </div>
                                <div class="mb-3">
                                    <label for="NamaProduk" class="form-label">NamaProduk</label>
                                    <input type="text" style="background-color: rgba(0, 46, 107, 0.60)"
                                        class="form-control" placeholder="Masukkan Nama Produk" name="NamaProduk"
                                        required id="NamaProduk">
                                </div>
                                <div class="mb-3">
                                    <label for="Harga" class="form-label">Harga</label>
                                    <input type="number" style="background-color: rgba(0, 46, 107, 0.60)"
                                        class="form-control" placeholder="Masukkan Harga Produk" name="Harga"
                                        required id="Harga">
                                </div>
                                <div class="mb-3">
                                    <label for="Stok" class="form-label">Stok</label>
                                    <input type="number" style="background-color: rgba(0, 46, 107, 0.60)"
                                        class="form-control" placeholder="Masukkan Stok Produk" name="Stok"
                                        required id="Stok">
                                </div>
                ...
                <center>
                    <table class="table text-center table-sm table-hover table-bordered table-striped-columns"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="background-color: rgba(0, 102, 98, 0.748)"><i class="bi bi-file-text"
                                        style="background-color: rgba(0, 102, 98, 0.1)"></i> NamaProduk
                                </th>
                                <th style="background-color: rgba(0, 102, 98, 0.748)"><i class="bi bi-currency-dollar"
                                        style="background-color: rgba(0, 102, 98, 0.1)"></i> Harga</th>
                                <th style="background-color: rgba(0, 102, 98, 0.748)"><i class="bi bi-archive"
                                        style="background-color: rgba(0, 102, 98, 0.1)"></i> Stok</th>
                        ...
                        @foreach ($produk as $item)
                            <tr>
                                <th style="background-color: rgba(2, 36, 78, 0.2)">{{ $item->ProdukID }}</th>
                                <td style="background-color: rgba(2, 36, 78, 0.2)">{{ $item->NamaProduk }}</td>
                                <td style="background-color: rgba(2, 36, 78, 0.2)">Rp.{{ $item->Harga }}</td>
                                <td style="background-color: rgba(2, 36, 78, 0.2)">{{ $item->Stok }}</td>
                                <td style="background-color: rgba(2, 36, 78, 0.2)">
                                    <center><a href="/produk/{{ $item->ProdukID }}/edit"
                                            class="btn btn-warning my-1 mx-2"><i class="bi bi-pencil-square"></i>
                                            Edit</a></center>
                                </td>
                                <td style="background-color: rgba(2, 36, 78, 0.2)">
                                    <center>
                                        <form action="/produk/{{ $item->ProdukID }}" method="POST">
                                            @csrf
                                            @method('delete')
                                            <button type="submit"
                                                onclick="return confirm('Are you sure want to delete {{ $item->NamaProduk }} with ID {{ $item->ProdukID }}?')"
        ...
        <div class="container pt-4">
            {{ $produk->appends(compact('perPage'))->links() }}
        </div>
    </main>
    ...
    <script>
        document.getElementById('pagination').onchange = function() {
            window.location = "{!! $produk->url(1) !!}&perPage=" + this.value;
        };
    </script>
